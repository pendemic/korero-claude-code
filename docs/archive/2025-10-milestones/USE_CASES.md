...
**Date**: 2025-10-01
**Purpose**: Define actors, goals, and scenarios for Korero autonomous development system

---

## System Overview

**System Name**: Korero - Autonomous AI Development Loop
**System Goal**: Complete software project implementation with minimal human intervention and token waste
**Primary Actor**: Korero (bash script orchestrating Claude Code)
...
**Type**: Human
**Goal**: Initiate Korero, review results, intervene when needed
**Capabilities**:
- Create PROMPT.md and fix_plan.md
- Start/stop Korero execution
...
- Cannot modify files while Korero is running
...
1. Korero reads PROMPT.md
2. Korero checks circuit breaker state (must be CLOSED or HALF_OPEN)
3. Korero verifies rate limit allows execution
4. Korero executes Claude Code with PROMPT.md
...
8. Claude Code outputs KORERO_STATUS block
9. Korero analyzes Claude's response (analyze_response)
10. Korero updates .exit_signals file (update_exit_signals)
11. Korero records loop result in circuit breaker (record_loop_result)
12. Korero increments call counter
13. Korero logs completion to status.json and logs/
14. Korero continues to next loop (if no exit condition)

**Extensions** (Alternative Flows):

**2a. Circuit breaker is OPEN**:
- 2a1. Korero displays circuit breaker status
- 2a2. Korero shows user guidance (check logs, reset, etc.)
- 2a3. Korero exits with exit code 1
- USE CASE ENDS

**3a. Rate limit exceeded**:
- 3a1. Korero calculates time until next hour reset
- 3a2. Korero displays countdown timer
- 3a3. Korero waits for reset
- 3a4. Korero continues at step 4

**3b. API 5-hour limit reached**:
- 3b1. Korero detects "rate limit" error in Claude output
- 3b2. Korero prompts user: retry or exit?
- 3b3a. User chooses retry: wait 5 minutes, go to step 4
- 3b3b. User chooses exit: Korero exits gracefully
- USE CASE ENDS

**4a. Claude Code execution fails**:
- 4a1. Korero logs error to logs/korero_error.log
- 4a2. Korero updates status.json with "failed" status
- 4a3. Korero continues to next loop (retry)
- 4a4. If 5 consecutive failures: circuit breaker opens
- Continue at step 2

**9a. Response analysis detects EXIT_SIGNAL=true**:
- 9a1. Korero logs successful completion
- 9a2. Korero updates status.json with "complete" status
- 9a3. Korero displays completion summary
- 9a4. Korero exits with exit code 0
- USE CASE ENDS

**11a. Circuit breaker opens (no progress detected)**:
- 11a1. Korero logs circuit breaker opening
- 11a2. Korero updates status.json with "circuit_open" status
- 11a3. Korero displays guidance to user
- 11a4. Korero exits with exit code 1
- USE CASE ENDS
...
**Primary Actor**: Korero (via response_analyzer.sh)
...
**Main Success Scenario**:
1. Korero reads Claude Code output file
2. Korero checks for structured KORERO_STATUS block
3. Korero finds STATUS: COMPLETE and EXIT_SIGNAL: true
4. Korero sets confidence score to 100
5. Korero sets exit_signal to true in .response_analysis
6. Korero updates .exit_signals with done_signals array
7. Korero triggers graceful exit in next loop check

**Extensions**:

**2a. No structured output found**:
- 2a1. Korero searches for natural language completion keywords
- 2a2. If found: add +10 to confidence score
- 2a3. Korero checks for "nothing to do" patterns
- 2a4. If found: add +15 to confidence score, set exit_signal=true
- Continue at step 6

**3a. STATUS shows IN_PROGRESS**:
- 3a1. Korero checks WORK_TYPE field
...
- 3b1. Korero increments blocked_loops counter
...
**Primary Actor**: Korero (via circuit_breaker.sh)
...
**Main Success Scenario**:
1. Korero initializes circuit breaker to CLOSED state
2. After each loop, Korero calls record_loop_result()
3. Korero counts files_changed from git diff
4. Korero detects has_errors from Claude output
5. Korero calculates output_length
...
9. Korero continues to next loop
...
- 6b3. Korero logs "monitoring mode" warning
- Continue at step 9

**6c. No files changed for 3rd consecutive loop**:
- 6c1. consecutive_no_progress = 3
- 6c2. Circuit breaker transitions to OPEN
- 6c3. Korero displays halt message with guidance
- 6c4. Korero exits with exit code 1
- USE CASE ENDS
...
- 7a3. Korero logs "circuit recovered"
- Continue at step 9
...
**Preconditions**:
- Korero is executing development loops
...
**Main Success Scenario**:
1. Korero checks current hour (YYYYMMDDHH format)
2. Korero reads .last_reset timestamp
3. Current hour matches last_reset (same hour)
4. Korero reads .call_count
5. call_count is 45 (< 100 limit)
6. Korero allows execution
7. Korero increments call_count to 46
8. Korero writes updated count to .call_count
9. Execution proceeds

**Extensions**:

**3a. New hour detected (hour changed)**:
- 3a1. Korero resets call_count to 0
- 3a2. Korero writes current hour to .last_reset
- 3a3. Korero logs "call counter reset for new hour"
- Continue at step 5

**5a. call_count equals or exceeds limit (100)**:
- 5a1. Korero calculates seconds until next hour
- 5a2. Korero displays countdown: "Rate limit reached. Waiting HH:MM:SS..."
- 5a3. Korero sleeps for calculated duration
- 5a4. Korero resets counter (go to step 3a1)
- Continue at step 6

**5b. Claude returns API rate limit error**:
- 5b1. Korero detects "rate_limit_error" in output
- 5b2. Korero prompts: "API 5-hour limit reached. Retry? (y/n)"
- 5b3a. User enters 'y': Korero waits 5 minutes, retries
- 5b3b. User enters 'n': Korero exits gracefully
- USE CASE ENDS
...
**Primary Actor**: korero-monitor.sh
**Stakeholders**: Human Developer (wants real-time visibility)
**Preconditions**:
- Korero is running (korero_loop.sh)
- korero-monitor started in separate terminal
...
**Main Success Scenario**:
1. User starts korero-monitor.sh in separate terminal
...
- 3a1. Monitor displays "Waiting for Korero to start..."
...
**Frequency**: Continuous until Korero exits
...
**Stakeholders**: Korero (needs manual reset to continue)
**Preconditions**:
- Circuit breaker is OPEN
- Korero has halted execution
- User has reviewed logs and identified issue

**Success Guarantee**:
- Circuit breaker reset to CLOSED state
- Counters reset to 0
- Korero can resume execution
- Reset reason logged

**Main Success Scenario**:
1. User identifies circuit breaker opened (from korero-monitor or logs)
2. User reviews logs/korero.log to understand cause
3. User fixes underlying issue (updates fix_plan.md, fixes error, etc.)
4. User runs: `korero --reset-circuit`
5. Korero loads circuit_breaker.sh functions
6. Korero calls reset_circuit_breaker("Manual reset by user")
7. Korero sets state to CLOSED in .circuit_breaker_state
8. Korero resets all counters to 0
9. Korero logs "Circuit breaker reset to CLOSED state"
10. Korero displays success message
11. User can now restart Korero execution

**Extensions**:

**2a. User cannot determine cause from logs**:
- 2a1. User runs: `korero --status` for additional info
...
| **Loop** | One iteration of Korero executing Claude Code |
