...
#   ALLOWED_TOOLS="@conservative,Bash(pytest)"
...
# Returns: expanded tool list, or the input unchanged if not a preset
expand_tool_preset() {
    local preset="${1:-}"
...
expand_allowed_tools() {
    ...
    for part in "${parts[@]}"; do
        ...
        if [[ "$part" == "@"* ]]; then
            # It's a preset reference — expand it
            local expanded
            ...
            # Regular tool — pass through unchanged
            if [[ -n "$part" ]]; then
                if [[ -n "$result" ]]; then
                    result="$result,$part"
                else
